...
{ pkgs, ... }: {
  ...
  env = {
    ...
    # Client-side Firebase Keys (Safe to commit)
    ...
    # Server-side Keys
    # NOTE: GitHub will block these if they contain real secrets.
    # Use empty strings here and manually add to .env.local if Push Protection fails.
  ...
  idx = {
    ...
    workspace = {
      ...
      onStart = {
        # 2025 Smart Script: Rebuilds .env.local every time the workspace starts
        sync-env = ''
