...
import { FullUserProfile, FounderProfile, TalentProfile, Startup, Profile, UserRole, TalentSubRole } from "./types";
...
export async function getSmartMatches(user: FullUserProfile) {
    ...
    if (user.role === 'founder') {
        ...
        userProfileDesc += `Investor with interests in ${user.profile.investmentInterests.join(', ')}. Thesis: ${user.profile.thesis}`;
    } else if (user.role === 'talent' && 'skills' in user.profile) {
        userProfileDesc += `Talent with skills in ${user.profile.skills?.join(', ')}. Experience: ${user.profile.experience}`;
    }
