...
export default function MatchesPageClient() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            const allUsers = usersSnap.docs.map(doc => toSerializable(doc.data()) as FullUserProfile).filter(u => u.id !== currentUser.id);
            
            const startupsSnap = await getDocs(collection(db, "startups"));
            setStartups(startupsSnap.docs.map(doc => toSerializable(doc.data()) as Startup));
