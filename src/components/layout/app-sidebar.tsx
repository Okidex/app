...
export function AppSidebar() {
  ...
  const isSeekingCoFounder = isTalent && (user.profile as TalentProfile).isSeekingCoFounder;
    ...
    <Sidebar>
      ...
      <SidebarMenu className="p-2 flex-1">
        {menuItems.map(({ href, label, icon, premium, notificationType }) => {
          const notificationCount = notifications.filter(n => n.type === notificationType).length;

          return (
            <SidebarMenuItem key={href}>
              <SidebarMenuButton
                asChild
                isActive={pathname.startsWith(href) && (href !== '/profile' || pathname === href)}
                tooltip={label}
              >
                <Link href={href}>
