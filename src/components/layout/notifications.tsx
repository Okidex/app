        ...
        const unsubscribe = onSnapshot(q, async (snapshot) => {
            ...
            if (senderIds.length > 0) {
                ...
                if (idsToFetch.length > 0) {
                    const usersQuery = query(collection(db, "users"), where("id", "in", idsToFetch));
                        ...
                        return (
                             <div key={notification.id}>
                                <Link href={notification.link} className={cn(
                                    ...
                                    {sender ? <UserAvatar name={sender.name} avatarUrl={sender.avatarUrl} /> : <div className='p-2 bg-secondary rounded-full'>{getIcon(notification.type)}</div>}
                                    <div className="flex-1 space-y-1">
                                        <p className="text-sm" dangerouslySetInnerHTML={{ __html: sender ? notification.text.replace(sender.name, `<strong>${sender.name}</strong>`) : notification.text }}></p>
                                        <p className="text-xs text-muted-foreground">
                                            {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true })}
                                        </p>
                                    </div>
                                    {!notification.isRead && <div className="w-2 h-2 rounded-full bg-primary mt-1" />}
                                </Link>
